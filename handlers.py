...
async def get_missing_channels(bot: Bot, user_id: int) -> list:
    ...
    missing = []
    placeholders = {"@AMON_Channel1", "@AMON_Channel2", "@AMON_Channel3", "@AMON_Channel4",
                    "AMON_Channel1", "AMON_Channel2", "AMON_Channel3", "AMON_Channel4"}
    for channel in FORCE_JOIN_CHANNELS:
        # Skip default placeholders so bot still responds before real channels are set
        if channel in placeholders or not channel or channel.strip() == "":
            continue
...
async def force_join_required(bot: Bot, user_id: int) -> bool:
    ...
    if is_admin(user_id):
        return False
    # If no real channels configured, do not force join
    placeholders = {"@AMON_Channel1", "@AMON_Channel2", "@AMON_Channel3", "@AMON_Channel4",
                    "AMON_Channel1", "AMON_Channel2", "AMON_Channel3", "AMON_Channel4"}
    real_channels = [c for c in FORCE_JOIN_CHANNELS if c and c not in placeholders]
